...
classifier.add(Convolution1D(activation="relu",
                        input_shape=(1000, 4),
                        padding="valid", strides=1,
                        filters=320, kernel_size=26))

classifier.add(MaxPooling1D(strides=13, pool_size=13))
...
classifier.add(Dense(input_dim=75*640, units=925))
classifier.add(Activation('relu'))

classifier.add(Dense(input_dim=925, units=919))
classifier.add(Activation('sigmoid'))

print('compiling model')
classifier.compile(loss='binary_crossentropy', optimizer='adam', metrics=['cosine'])


#classifier = MLPClassifier(solver='lbfgs', max_iter=1000,random_state=1)
